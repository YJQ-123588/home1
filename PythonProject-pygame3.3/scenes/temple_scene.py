...
class TempleScene:
    ...
    def update(self):
        ...
        if self.fighting and self.current_fight:
            ...
            if self.current_fight.is_done():
                if self.current_fight.is_avoided():
                    # 避战：扣除20血量，怪物不被击败
                    self.player_hp = self.current_fight.player_hp
                    if self.current_cattle:
                        # 同步怪物血量（避战时怪物也受到了伤害）
                        self.current_cattle.hp = self.current_fight.monster_hp
                        self.current_cattle.paused = False
                    self.avoid_cooldown = 60
                elif self.current_fight.is_win():
                    if self.current_cattle:
                        self.defeated_monsters.add(self.current_cattle.name)
                        self.current_cattle.hp = 0
                        self.current_cattle.alive = False
                        self.current_cattle.visible = False
                    self.player_hp = self.current_fight.player_hp
                else:
    ...
    def draw(self, screen):
        ...
        # 绘制小地图
        self.minimap.draw(screen, self.camera_x, self.camera_y)

        # 绘制状态栏
        self.status_bar.draw(screen, self.player_hp, self.player_max_hp,
                            self.player_atk, self.ultimate_unlocked)
