...
class VillageScene:
    ...
    def _check_npc_collision(self):
        ...
        for elder in self.elders:
            ...
            if dist < 60:
                # 记录对话前的状态
                was_upgrade_given = elder.upgrade_given
                ...
                # 只有第一次获得增益时才生效
                if not was_upgrade_given and elder.upgrade_given:
                    if elder.get_upgrade_type() == "hp_boost":
                        self.player.max_hp += 50
                        self.player.hp = min(self.player.hp + 50, self.player.max_hp)
                    elif elder.get_upgrade_type() == "ultimate":
                        self.ultimate_unlocked = True
                elif elder.get_upgrade_type() == "heal":
    ...
    def draw(self, screen):
        ...
        # 绘制状态栏
        self.status_bar.draw(screen, self.player.hp, self.player.max_hp,
                            PLAYER_BASE_ATK, self.ultimate_unlocked)
